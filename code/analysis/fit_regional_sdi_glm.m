...
% Description: Fit ROI-wise age/sex-adjusted group models for structural decoupling index.
% Usage: Run after compute_sdi_for_cohort has generated SDI_all.
% Outputs: Outputs ROI-wise SDI statistics and summary MAT/TXT files.
...
%% ========= load SDI =========
load(sdi_mat_file, 'matched', 'SDI_all');
...
tag_sdi = string(group_num) + "_" + sub_id;
...
%% ========= match =========
[common_tag, ia, ib] = intersect(tag_sdi, C.tag, 'stable');
...
fprintf('Matched subjects for SDI stats: %d\n', numel(common_tag));
...
save(out_mat, 'Tout', 'SDI', 'group_name', 'sub_id', 'group_num', 'Cm');

fprintf('\nOutput file/path/to/project', out_summary_txt, out_mat);
